...
// Page with the same page endpoint portfolio to view or edit portfolio
...
// Template where an user can view a particular portfolio
template viewPortfolio(portfolio: Portfolio) {
	...
	if(portfolio.assets.length > 0) {
		row[class="portfolio mt-4", portfolio-id=""+portfolio.id] {
...
// Template where an user can edit a particular portfolio
				...
				card[class="border-lighter"] {
					...
					card_body {
						...
						placeholder ph_general {
							form_row_validation {
								templateSuccess([generalSaveSuccess])
							}
							...
							form_row {
								col("col-12 d-flex justify-content-between") {
									...
									}[class="btn btn-sm btn-danger", id="btn-delete-portfolio"] { 
										"Delete Portfolio" 
									}
